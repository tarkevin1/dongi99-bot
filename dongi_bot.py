# dongi_bot.py (نسخه نهایی با ذخیره نام کاربران)
import logging
import os
...
print("--- STARTING FINAL BOT VERSION WITH USERNAME LOGGING ---")
...
class User(Base):
    ...
    first_name = Column(String) # <-- ستون جدید برای نام
    username = Column(String) # <-- ستون جدید برای نام کاربری
...
# ... (دکوریتورها و توابع کیبورد بدون تغییر)
...
def admin_only(func):
    ...
    async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        chat_id = update.effective_chat.id
        if chat_id != ADMIN_CHAT_ID: await update.message.reply_text("شما اجازه دسترسی ندارید."); return
        return await func(update, context, *args, **kwargs)
    return wrapped
def main_menu_reply_keyboard():
    keyboard = [["💳 ثبت هزینه جدید", "📊 گزارش کامل"], ["🧾 لیست هزینه‌ها", "🗑️ حذف یک هزینه"], ["👥 مدیریت افراد"]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ذخیره کردن کاربر با اطلاعات کامل
    ...
    if not db_user:
        ...
        # آپدیت اطلاعات در صورتی که کاربر نام خود را تغییر داده باشد
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for user in all_users:
        ...
        message += f"👤 <b>{user_info}</b>\n"
        message += f"<b>Chat ID:</b> <code>{user.chat_id}</code> - {status}\n"
        message += "--------------------\n"
    
    await update.message.reply_html(message)

# ... (تمام توابع دیگر ربات بدون تغییر در اینجا قرار می‌گیرند) ...
# (برای کوتاهی، کدهای تکراری حذف شده‌اند)
@check_if_blocked
async def add_expense_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    target_message = update.message
    people = session.query(Person).all()
    if not people: await target_message.reply_text('...'); return ConversationHandler.END
...
async def my_expenses(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not all_expenses: await update.message.reply_text("..."); return
...
async def manage_people_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("...", reply_markup=main_menu_reply_keyboard())
@check_if_blocked
async def delete_expense_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_html("...", reply_markup=main_menu_reply_keyboard())
async def send_notification_to_all(message: str, context: ContextTypes.DEFAULT_TYPE, notifier_chat_id: int):
    ...
    for user in all_users:
        ...
        try: await context.bot.send_message(chat_id=user.chat_id, text=message)
        except Forbidden:
            session.delete(user)
            session.commit()
        except Exception as e: logger.error(f"Could not send message to {user.chat_id}: {e}")
@check_if_blocked
async def add_person(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if session.query(Person).filter_by(name=new_name).first(): await update.message.reply_text(f'"{new_name}" موجود است.'); return
...
async def del_person(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if person:
        ...
        await update.message.reply_text(f'🗑️ "{name_to_delete}" حذف شد.')
...
def main() -> None:
    ...
    if not TOKEN: print("خطا: توکن یافت نشد."); return
